...
class HomeController extends FrontController
{
	...
	protected function footerImages()
	{
        $data = \App\Models\Setting::where('key', 'above_footer')->first();
        return  view('vendor.admin.footr', ['data'=>$data]);;
	}
	public function footerImages_save(Request $request){
	    $data = \App\Models\Setting::where('key', 'above_footer')->first();
	    $arr=array();
	    foreach($data->value as $img){
	        if($img['key']==$request->id){

                if($request->image){
                    $image = 'img_' . time() . '.' . $request->image->extension();
                    $request->image->move(public_path('images'), $image);
                }
                else{
                    $image=$img['image'];
                }
                $arr[]=json_decode('{"key":"'.$request->id.'","image":"'.$image.'","text":"'.$request->title.'","status":"'.$request->status.'"}','assoc');
            }
	        else{
                $arr[]=json_decode('{"key":"'.$img['key'].'","image":"'.$img['image'].'","text":"'.$img['text'].'","status":"'.$img['status'].'"}','assoc');
            }
//	        var_dump($arr);
...
//		view()->share('rent_data', $rent_data);
	}
    ...
    public function bannerDelete(Request $request){
		...
		public function profileDetails($id){
			$posts = Post::where('user_id', $id)->with('reviews','pictures','category')->orderBy('featured', 'desc')->get();
			// dd($posts);
			$user = User::where('id', $id)->first();
			$user_reviews = Review::where('reviewer_id', $id)->with('post')->get();
//			 dd($user);
			return view('post.profile-details', compact('user', 'posts', 'user_reviews'));
		}
