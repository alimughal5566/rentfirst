...
function check_limit($user_id, $cat_id){
    $post_count = \App\Models\Post::where('user_id',$user_id)->where('category_id', $cat_id)->get()->count();
    $category = \App\Models\Category::where('id', $cat_id)->first();
    $data = '';
    if ($post_count == $category->post_limit){
        $data = false;
    }elseif ($post_count > $category->post_limit){
        $data = false;
    }elseif ($post_count < $category->post_limit){
        $data = true;
    }
    if ($data == false && auth()->user()->is_paid == 1){
        $package = \App\Models\LimitPackage::where('id', auth()->user()->packageid)->first();
        $total = $category->post_limit + $package->post_limit;
...
function sendRequest($rent_date){
    $out='';
    if ($rent_date == ''){
        $rent_date = 'today';
        $out .= '<div class="p-3"> Start Date: <input name="start_date" id="start_date" class="form-control" type="date" required>';
        $out .= 'End Date: &nbsp;<input name="end_date" type="date" class="form-control" id="end_date" required> </div>
        <script>
            var today = new Date().toISOString().split(\'T\')[0];
            console.log(today)
            document.getElementsByName("start_date")[0].setAttribute(\'min\', '.$rent_date.');
            var today = new Date().toISOString().split(\'T\')[0];
            document.getElementsByName("end_date")[0].setAttribute(\'min\', '.$rent_date.');
        </script>';
    }else{
        $rent_date = date('Y-m-d', strtotime($rent_date."+1 day"));
        $out .= '<div class="p-3"> Start Date: <input name="start_date" id="start_date" class="form-control" type="date" required>';
        $out .= 'End Date: &nbsp;<input name="end_date" type="date" class="form-control" id="end_date" required> </div>
        <script>
            var today = new Date().toISOString().split(\'T\')[0];
...
function pendingRequest(){
    $btnClass = '';
    $out='';
    $out .= '<a href="' . auth()->id() . '" class="btn btn-default' . $btnClass . 'disabled" data-toggle="modal">';
    $out .= '<i class="fa fa-paper-plane"></i> ';
    $out .= "Request Pending";
    $out .= '</a>';
    return $out;
}
