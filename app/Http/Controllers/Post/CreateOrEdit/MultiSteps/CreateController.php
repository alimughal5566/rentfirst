...
//                $check = check_limit(auth()->user()->id, $request->category_id);
//                \Session::flash('message', 'Your post limit has been exceed. Kindly buy a package!');
...
//        }
	...
	public function finish($tmpToken)
	{
        ...
        $post_detail = Post::where('tmp_token', $tmpToken)->first();
        if ($post_detail && auth()->check()){
            if (isset($post_detail->category_id)){
                $check = check_limit(auth()->user()->id, $post_detail->category_id);
                if($check == false){
                    
                    \Session::flash('message', 'Your post limit has been exceed for this category. Kindly buy a package!');
                    \Session::flash('alert-class', 'alert-danger');
                    $data['showmsg'] = 'error';
