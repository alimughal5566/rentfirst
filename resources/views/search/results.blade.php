                    ...
                    <div class="category-list{{ ($contentColSm == 'col-md-12') ? ' noSideBar' : '' }}">
                                ...
                                <li>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a href="{!! qsUrl(request()->url(), request()->except(['orderBy', 'distance']), null, false) !!}"
                                                   rel="nofollow">
                                                    {{ t('Sort by') }}
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{!! qsUrl(request()->url(), array_merge(request()->except('orderBy'), ['orderBy'=>'priceAsc']), null, false) !!}"
                                                   rel="nofollow">
                                                    {{ t('price_low_to_high') }}
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{!! qsUrl(request()->url(), array_merge(request()->except('orderBy'), ['orderBy'=>'priceDesc']), null, false) !!}"
                                                   rel="nofollow">
                                                    {{ t('price_high_to_low') }}
                                                </a>
                                            </li>
                                            @if (request()->filled('q'))
                                                <li>
                                                    <a href="{!! qsUrl(request()->url(), array_merge(request()->except('orderBy'), ['orderBy'=>'relevance']), null, false) !!}"
                                                       rel="nofollow">
                                                        {{ t('Relevance') }}
                                                    </a>
                                                </li>
                                            @endif
                                            <li>
                                                <a href="{!! qsUrl(request()->url(), array_merge(request()->except('orderBy'), ['orderBy'=>'date']), null, false) !!}"
                                                   rel="nofollow">
                                                    {{ t('Date') }}
                                                </a>
                                            </li>
                                            @if (isset($city, $distanceRange) and !empty($city) and !empty($distanceRange))
                                                @foreach($distanceRange as $key => $value)
                                                    <li>
                                                        <a href="{!! qsUrl(request()->url(), array_merge(request()->except('distance'), ['distance' => $value]), null, false) !!}"
                                                           rel="nofollow">
                                                            {{ t('around_x_distance', ['distance' => $value, 'unit' => getDistanceUnit()]) }}
                                                        </a>
                                                    </li>
                       ...
                       @php $user=Request::segment(2) @endphp
                        ...
                        @foreach($post_reviews as $post_review)
                            ...
                            @endforeach
                        @endforeach
                        <h3 class="text-left float-left font-weight-bold">Reviews <small>{{($total>0)?round($rating/$total,1):'N/a'}} ({{$total}})</small></h3>
                                ...
                                @foreach($post_review->reviews as $review)
                                        ...
                                        <div class="card mb-3 adds-wrapper row no-margin justify-content-center " >
                                            <div class="row no-gutters">
                                                <div class="col-md-1">
                                                    <img src="{{url('/images/site/cat.png')}}"  height="50px"  class="ml-3 mt-3 "  alt="...">
                                                </div>
                                                <div class="col-md-10">
                                                    <div class="card-body pb-0">
                                                        <p class="card-text float-right text-right"><small class="text-muted">{{$review->created_at->diffForHumans()}}</small></p>
                                                        <h5 class="card-title p-0 m-0">{{$review->user->name}}</h5>
                                                        <p class="card-text p-0 m-0">{{$post_review->title}}</p>
...
{{--                                <a name="{!! qsUrl(request()->url(), request()->except(['_token', 'location']), null, false) !!}"--}}
                    ...
                    <div class="post-promo text-center mb-5">
                        ...
                        @if (!auth()->check() and config('settings.single.guests_can_post_ads') != '1')
                            <a href="#quickLogin" class="btn btn-border btn-post btn-add-listing"
                               data-toggle="modal">{{ t('start_now') }}</a>
                        @else
                            <a href="{{ \App\Helpers\UrlGen::addPost() }}"
                               class="btn btn-border btn-post btn-add-listing">{{ t('start_now') }}</a>
                        @endif
                    </div>
